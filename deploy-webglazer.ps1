# Quick deployment script for webglazer project
# Run this AFTER authenticating with: gcloud auth login matvei.medvedev@startschool.org

$ProjectId = "webglazerprototype-486313"
$ServiceName = "website-feature-finder"
$Region = "us-central1"

Write-Host "Deploying to Google Cloud Run (webglazer project)..." -ForegroundColor Cyan
Write-Host ""
...
if ($authCheck -match "No credentialed") {
    Write-Host "ERROR: Not authenticated. Please run: gcloud auth login matvei.medvedev@startschool.org" -ForegroundColor Red
    exit 1
}
...
gcloud builds submit --tag "gcr.io/$ProjectId/$ServiceName" --timeout=20m
